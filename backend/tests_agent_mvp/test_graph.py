...
Run with: pytest backend/tests_agent_mvp/ -v
...
from agent_mvp.validators import (
...
def test_do_selector_returns_valid_task_id(sample_candidates, sample_constraints):
    ...
    """
    with patch("agent_mvp.llm_do_selector.get_gemini_client") as mock_gemini:
...
def test_do_selector_fallback_on_invalid_task_id(sample_candidates):
    """
    Test that DoSelector falls back when Gemini returns invalid task_id.
    """
    with patch("agent_mvp.llm_do_selector.get_gemini_client") as mock_gemini:
        mock_client = Mock()
...
def test_do_selector_handles_invalid_json(sample_candidates):
    """
    Test that DoSelector handles invalid JSON from Gemini gracefully.
    """
    with patch("agent_mvp.llm_do_selector.get_gemini_client") as mock_gemini:
        mock_client = Mock()
        # Simulate Gemini returning invalid JSON
        mock_client.generate_json_response.side_effect = ValueError(
            "Invalid JSON"
        )
        mock_gemini.return_value = mock_client
...
def test_coach_output_is_short(sample_candidates):
    ...
    with patch("agent_mvp.llm_coach.get_gemini_client") as mock_gemini:
        mock_client = Mock()
        mock_client.generate_json_response.return_value = {
            "title": "Let's fix it!",
...
async def test_graph_state_flows_through_nodes(sample_candidates, sample_constraints):
    ...
    with patch("agent_mvp.graph.get_supabase") as mock_supabase:
    ...
    # Test llm_select_do
    with patch("agent_mvp.graph.select_task") as mock_select:
    ...
    # Test llm_coach
    with patch("agent_mvp.graph.generate_coaching_message") as mock_coach:
...
def test_end_to_end_agent_mvp_flow(sample_candidates, sample_constraints):
    ...
    """
    with patch("agent_mvp.llm_do_selector.get_gemini_client") as mock_gemini_1:
        with patch("agent_mvp.llm_coach.get_gemini_client") as mock_gemini_2:
            # Mock DoSelector
            mock_selector_client = Mock()
            mock_selector_client.generate_json_response.return_value = {
                "task_id": sample_candidates[0].id,
                "reason_codes": ["priority_high"],
                "alt_task_ids": [sample_candidates[1].id],
            }
            mock_gemini_1.return_value = mock_selector_client

            # Mock Coach
            mock_coach_client = Mock()
            mock_coach_client.generate_json_response.return_value = {
                "title": "Time to code!",
                "message": "Let's fix this bug.",
                "next_step": "Open IDE.",
            }
            mock_gemini_2.return_value = mock_coach_client
