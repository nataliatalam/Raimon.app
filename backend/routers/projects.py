...
async def get_user_project(project_id: str, user_id: str):
...
async def get_project(
    ...
    # details_response.data is a list, get first item or None
...
async def update_project_goals(
    ...
    if existing.data:
        ...
        response = (
            supabase.table("project_details")
            .insert({"project_id": project_id, "goals": request.goals})
            .execute()
        )
...
async def update_project_resources(
    ...
    if existing.data:
        response = (
            supabase.table("project_details")
            .update({"resources": resources_data})
            .eq("project_id", project_id)
            .execute()
        )
    else:
        response = (
            supabase.table("project_details")
            .insert({"project_id": project_id, "resources": resources_data})
            .execute()
        )
...
async def update_project_timeline(
    ...
    if project_update:
        supabase.table("projects").update(project_update).eq("id", project_id).execute()
...
async def update_project_stakeholders(
    ...
    if existing.data:
        response = (
            supabase.table("project_details")
            .update({"stakeholders": stakeholders_data})
            .eq("project_id", project_id)
            .execute()
        )
    else:
        response = (
            supabase.table("project_details")
            .insert({"project_id": project_id, "stakeholders": stakeholders_data})
            .execute()
        )
