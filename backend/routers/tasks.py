...
async def verify_project_access(project_id, user_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to verify project access: {e}")
        raise HTTPException(
...
async def list_project_tasks(
    ...
    try:
        ...
        logger.error(f"Error in list_project_tasks: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def delete_task(
    ...
    try:
        ...
        if subtasks.data:
            # Cascade delete subtasks (verify ownership)
            for subtask in subtasks.data:
                supabase.table("tasks").delete().eq("id", subtask["id"]).eq("user_id", current_user["id"]).execute()
...
async def pause_task(
    ...
    try:
        ...
        logger.error(f"Error in pause_task: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def report_intervention(
    ...
    try:
        ...
        logger.error(f"Error in report_intervention: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to report intervention",
        )
