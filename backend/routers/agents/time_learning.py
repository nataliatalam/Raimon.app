...
async def track_time_data(
    ...
    try:
        ...
        estimated = request.estimated_duration or task_data.get("estimated_duration")
        accuracy = None
        if estimated:
        ...
        logger.error(f"Error in track_time_data: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to track time data",
        )
...
async def get_time_predictions(
    ...
    try:
        ...
        logger.error(f"Error in get_time_predictions: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get time predictions",
        )
...
async def get_performance_insights(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get performance insights",
        )
