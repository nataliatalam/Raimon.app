...
@router.post("/analyze")
@track(name="priority_engine_analyze")
async def analyze_priorities(
    ...
    try:
        ...
        logger.error(f"Error in analyze_priorities: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to analyze priorities",
        )


@router.get("/recommendations")
@track(name="priority_engine_recommendations") 
async def get_recommendations(
    ...
    try:
        ...
        energy = checkin.data[0].get("energy_level", 5) if checkin.data else 5
...
@track(name="priority_engine_rerank")
async def rerank_tasks(
    ...
    try:
        ...
        logger.error(f"Error in rerank_tasks: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to rerank tasks",
        )
...
# @router.post("/analyze")
# @track(agent_name="priority_engine")
# async def analyze_priorities(
#     request: PriorityAnalyzeRequest,
#     current_user: dict = Depends(get_current_user),
# ):
...
#     tracker = AgentTracker("priority_engine_analyze")
...
#             "top_priority_score": prioritized[0]["priority_score"] if prioritized else 0,
...
#         await tracker.error(error=Exception("HTTP exception occurred"))
#         raise
#     except Exception as e:
#         logger.error(f"Error in analyze_priorities: {e}")
...
#     """Get task recommendations based on current context."""
#     # Initialize Opik tracker
#     tracker = AgentTracker("priority_engine_recommendations")
#     await tracker.start(input_data={
#         "user_id": current_user["id"],
#         "limit": limit
#     })
...
#         # Track successful completion
#         await tracker.complete(output_data={
#             "recommendations_count": len(prioritized[:limit]),
#             "user_energy_level": energy,
#             "top_recommendation_score": prioritized[0]["score"] if prioritized else 0
#         })
