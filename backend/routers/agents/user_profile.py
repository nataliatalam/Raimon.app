...
async def analyze_user_profile(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Failed to analyze user profile",
        )
...
async def get_profile_insights(
    ...
    try:
        ...
        logger.error(f"Error in get_profile_insights: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get profile insights",
        )
...
async def update_preferences(
    ...
    try:
        ...
        prefs_data = prefs.data[0] if prefs.data else None
        update_data = {}

        if request.work_patterns:
            current_work = prefs_data.get("work_style", {}) if prefs_data else {}
            update_data["work_style"] = {**current_work, **request.work_patterns}

        if request.energy_patterns:
            current_energy = prefs_data.get("energy_patterns", {}) if prefs_data else {}
