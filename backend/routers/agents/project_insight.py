...
async def get_completion_prediction(
    ...
    try:
        ...
        if project_data.get("target_end_date"):
            target_date = datetime.fromisoformat(project_data["target_end_date"] + "T23:59:59+00:00")
        ...
        logger.error(f"Error in get_completion_prediction: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get completion prediction",
        )
...
async def get_risk_analysis(
    ...
    try:
        ...
        logger.error(f"Error in get_risk_analysis: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
