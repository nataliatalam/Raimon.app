...
async def get_time_tracking(
    ...
    try:
        ...
        logger.error(f"Error in get_time_tracking: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get time tracking data",
        )
...
async def get_productivity_metrics(
    ...
    try:
        ...
        productivity_score = min(1.0, (
            (min(tasks_per_day / 5, 1) * 0.3) +
            (min(focus_per_day / 240, 1) * 0.3) +
            (estimation_accuracy * 0.2) +
            ((1 - min(total_interruptions / (len(sessions.data or []) * 3 + 1), 1)) * 0.2)
        ))
        ...
        logger.error(f"Error in get_productivity_metrics: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    try:
        ...
        for project in projects.data or []:
            ...
            if progress >= 100:
                ...
                proj_status = "almost_done"
            elif progress >= 25:
                proj_status = "on_track"
            else:
                proj_status = "just_started"
        ...
        logger.error(f"Error in get_project_performance: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get project performance",
        )
...
async def get_goal_progress(
    ...
    try:
        ...
        logger.error(f"Error in get_goal_progress: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
