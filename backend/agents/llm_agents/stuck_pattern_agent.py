...
from services.storage_service import get_session_patterns, get_active_do, save_stuck_episode, get_recent_stuck_episodes
...
def _enhance_microtasks_with_llm(session: WorkSession, base_tasks: List[str], llm_service: Optional[LLMService] = None) -> Optional[List[str]]:
    """Use LLM service to generate more specific microtasks."""
    try:
        ...
        response = service.generate_json(prompt, max_tokens=200)
